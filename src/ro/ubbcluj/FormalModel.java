        ...
        while (!Objects.equals(state.getStatus(), "t") &&
            ...
            if (isContinueStatus()) {
                if (isFinalValidState()) {
                    ...
                    expand();
                } else if (isTopInputStackTerminal() && inputStackMatchesInputString()) { //check if it matches input string
                    advance();
                } else {
                    momentaryInsuccess();
                }
            } else if (isBackStatus()) {
                if (isTopWorkingStackTerminal()) { //check if top of working stack is terminal
                    back();
                }
